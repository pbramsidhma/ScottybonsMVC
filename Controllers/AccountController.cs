    ...
    public class AccountController : RenderMvcController
    {
        ...
        public ActionResult EditProfileQuestions(ProfileQuestionsVm model, IEnumerable<HttpPostedFileBase> files)
        {
            ...
            else
            {
                ...
                mv.Add("STATUS", "Style intake done");
            ...
            try
            {
                ...
                if (model.ProfileQuestionsViewModel.NumberOfQuestions == model.ProfileQuestionsViewModel.QuestionNumber)
                {
                    ...
                    else
                    {
                        ...
                        //Send Email to Admin for answered questions
                        SendFillsStyleProfileEmailToAdmin(customer);
                        //Save/Subscribe new user on mailchimp list(9974196bc9).
                        var mailChimpAPIKey = GlobalConstants.MailChimpAPIKey;
                        var listId = GlobalConstants.MailChimpListId;
                        var mailChimpListId = string.IsNullOrEmpty(listId) ? mailChimpAPIKey : listId;
                        MailChimpManager mc = new MailChimpManager(mailChimpAPIKey);
                        MergeVar mv = new MergeVar();
                        mv.Add("FNAME", customer.FirstName);
                        mv.Add("LNAME", customer.LastName);
                        mv.Add("STATUS", "Style intake done");
                        var email = new EmailParameter()
                        {
                            Email = customer.Email
        ...
        /// </summary>
        private void SendFillsStyleProfileEmail(Customer customer, Models.Entities.Order order)
        {
