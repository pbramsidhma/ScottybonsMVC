...
namespace ScottybonsMVC.Controllers
{
    ...
    public class OrderPageController : SurfaceController
    {
        ...
        public ActionResult HandleScottyBoxAction(OrderVm model)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    var PeriodicValue = string.Empty;
                    var periodicId = model.PeriodicalScottyBoxSelected ? model.CustomerOrder.PeriodicalScottyBoxID : 0;
                    switch (periodicId)
                    {
                        case 1:
                        case 7:
                            PeriodicValue = "1 Month";
                            break;
                        case 2:
                        case 8:
                            PeriodicValue = "2 Months";
                            break;
                        case 3:
                        case 9:
                            PeriodicValue = "3 Months";
                            break;
                        default:
                            PeriodicValue = "None";
                            break;
                    ...
                    //Save/Subscribe new user on mailchimp list.
                    var mailChimpAPIKey = GlobalConstants.MailChimpAPIKey;
                    var listId = GlobalConstants.MailChimpListId;
                    var mailChimpListId = string.IsNullOrEmpty(listId) ? mailChimpAPIKey : listId;
                    MailChimpManager mc = new MailChimpManager(mailChimpAPIKey);
                    MergeVar mv = new MergeVar();
                    mv.Add("PERIODIC", PeriodicValue);
                    var customer = _scottybonsEComEntities.Customers.FirstOrDefault(c => c.CustomerID == GetSession.CustomerId);
                    var email = new MailChimp.Helper.EmailParameter()
                    {
                        Email = customer.Email
                    };

                    MailChimp.Helper.EmailParameter results = mc.Subscribe(mailChimpListId, email, mv, "html", false, true, true, false);
                ...
                if (ModelState.IsValid)
                {
                    ...
                    var customerOrder =
                            _scottybonsEComEntities.Orders.Where(m => m.CustomerID == customer.CustomerID)
                                ...
                                .FirstOrDefault();
                    var PreviousPeriodMonths = _scottybonsEComEntities.PeriodicalScottyBoxMasters.FirstOrDefault(p => p.PeriodicalScottyBoxID.Equals(customerOrder.PeriodicalScottyBoxID));
                        ...
                        customerOrder.NextPerodicalScottyBoxDate = model.IsSubscription
                                ? (customerOrder.NextPerodicalScottyBoxDate.Value.Date > DateTime.Now.Date)
                                    ? (customerOrder.PaymentDate.AddMonths(selectedPeriodMonths.PerodicalMonths.Value) > DateTime.Now.Date)
                                        ? customerOrder.PaymentDate.AddMonths(selectedPeriodMonths.PerodicalMonths.Value)
                                        : DateTime.Now.Date.AddDays(1).AddMonths(selectedPeriodMonths.PerodicalMonths.Value)
                                    : (customerOrder.NextPerodicalScottyBoxDate.Value.AddMonths(selectedPeriodMonths.PerodicalMonths.Value) > DateTime.Now.Date)
                                        ? customerOrder.NextPerodicalScottyBoxDate.Value.AddMonths(selectedPeriodMonths.PerodicalMonths.Value)
                                        : DateTime.Now.Date.AddDays(1).AddMonths(selectedPeriodMonths.PerodicalMonths.Value)
                                : customerOrder.NextPerodicalScottyBoxDate;
                        ...
                        //Save/Subscribe new user on mailchimp list.
                        var PeriodicValue = string.Empty;
                        var periodicId = customerOrder.PeriodicalScottyBoxID;
                        switch (periodicId)
                        {
                            case 1:
                            case 7:
                    ...
                    MergeVar mv = new MergeVar();
                    mv.Add("PERIODIC", PeriodicValue);
                    //var customerdtl = _scottybonsEComEntities.Customers.FirstOrDefault(c => c.CustomerID == GetSession.CustomerId);
                    var email = new MailChimp.Helper.EmailParameter()
                    {
                        Email = customer.Email
                    };
        ...
        public ActionResult ConfirmYourOrder(ConfirmYourOrderVm model)
        {
            try
            {
                model.PromoCodeRedeemed = model.PromoCodeRedeemed == null ? "false" : model.PromoCodeRedeemed.ToString();
                ...
                if (model.PaymentMethodId.Equals((int)OrderPaymentTypeEnum.GiftCard) || model.PaymentMethodId.Equals((int)OrderPaymentTypeEnum.PromoCode))
                {
                        ...
                        if (!string.IsNullOrEmpty(model.PromoCodes))
                        {
                            ...
                            foreach (var promocode in model.PromoCodes.Split(','))
                            {
                                ...
                                var promoCodeOrder = new PromoCodeOrder
                                {
                                    ...
                                    IsOnline = promoCodeObject.IsOnline,
                                    Status = true
                                };
                    ...
                    if (model.GiftCardRedeemed.ToLower().Equals("true"))
                    {
                        ...
                        var gcRedeemedValue = (Math.Round(Convert.ToDouble(float.Parse(model.GiftCardRedeemedValue, CultureInfo.InvariantCulture.NumberFormat)) * 100) / 100);
                ...
                else
                {
                        ...
                        if (model.PromoCodeRedeemed.ToLower().Equals("true"))
                        {
                            if (!string.IsNullOrEmpty(model.PromoCodes))
                            {
                                ...
                                foreach (var promocode in model.PromoCodes.Split(','))
                                {
                                    ...
                                    //transactionAmount = (transactionAmount - (Math.Round(Convert.ToDouble(splitPromoCodeNameValue[1], CultureInfo.InvariantCulture) * 100) / 100));
                                    transactionAmount = (promoCodeObject.IsOnline)
                                                        ? (transactionAmount - (Math.Round(Convert.ToDouble(splitPromoCodeNameValue[1], CultureInfo.InvariantCulture) * 100) / 100))
                                                        : transactionAmount;
                                    var promoCodeOrder = new PromoCodeOrder
                                    {
                                        PromoCode = splitPromoCodeNameValue[0],
                    ...
                    var checkOutRequest = new ScottybonsMVC.IcepayRestClient.Classes.Payment.CheckoutRequest
                    {
                        ...
                        URLError =
                        "http://" + HttpContext.Request.Url.Host + GlobalConstants.LanguageUrl +
                        GlobalConstants.IcePayTranError
                        + (model.GiftCardRedeemed.ToLower().Equals("true")
                            ? ("?giftcard=" + model.GiftCardRedeemed)
                            : string.Empty)
                        + (model.PromoCodeRedeemed.ToLower().Equals("true")
                            ? ((model.GiftCardRedeemed.ToLower().Equals("true") ? "&promocode=" : "?promocode=") + model.PromoCodeRedeemed)
                            : string.Empty),
        ...
        public JsonResult ValidatePromoCode(string promoCode)
        {
            try
            {
                ...
                var currentNode = Umbraco.TypedContent(3054);
                ...
                //};
                var promoCodeModel = new PromoCodeModel();
                promoCodeModel.IsFuture = Convert.ToDateTime(promoCodeObject.GetProperty("startDate").Value) > DateTime.Now.Date;
                promoCodeModel.Expired = Convert.ToDateTime(promoCodeObject.GetProperty("endDate").Value).Date < DateTime.Now.Date;

                if (promoCodeModel.IsFuture || promoCodeModel.Expired)
                {
                    return Json(promoCodeModel, JsonRequestBehavior.AllowGet);
                }

                promoCodeModel.PromoCode = promoCodeObject.GetProperty("promoCodeName").Value.ToString();
                promoCodeModel.PromoCodeValue = Convert.ToInt32(promoCodeObject.GetProperty("promoCodeValue").Value);
                promoCodeModel.AllowAllusers = Convert.ToBoolean(promoCodeObject.GetProperty("allowAllUsers").Value);
                promoCodeModel.OnlyOnce = Convert.ToBoolean(promoCodeObject.GetProperty("onlyOnce").Value);
                promoCodeModel.IsOnline = Convert.ToBoolean(promoCodeObject.GetProperty("isOnline").Value);
                promoCodeModel.IsPercentage = Convert.ToBoolean(promoCodeObject.GetProperty("isPercentage").Value);
                promoCodeModel.IsEnable = Convert.ToBoolean(promoCodeObject.GetProperty("isEnable").Value);
                promoCodeModel.StartDate = Convert.ToDateTime(promoCodeObject.GetProperty("startDate").Value);
                promoCodeModel.EndDate = Convert.ToDateTime(promoCodeObject.GetProperty("endDate").Value);
                promoCodeModel.Valid = true;
