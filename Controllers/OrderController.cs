    ...
    public class OrderController : RenderMvcController
    {
        ...
        public ActionResult PlanScottyBox(RenderModel model)
        {
            ...
            try
            {
                ...
                orderModel.HasSubscription =
                    _scottybonsEComEntities.Orders.Any(m => m.CustomerID == customerId && m.PeriodicalScottyBoxID != 0 && m.OrderStatusID == 4);
                orderModel.HasSubscription = false;//comment this for prod
        ...
        public ActionResult ConfirmYourOrder(RenderModel model)
        {
            try
            {
                ...
                var order = new ConfirmYourOrderVm(model.Content) { OrderNumber = GetSession.OrderNumber, PromoCodeRedeemed="false" };
            ...
            try
            {
                ...
                if (!ReferenceEquals(paymentId, null))
                {
                    ...
                    if (paymentResponse.Status == "OK")
                    {
                        ...
                        mv.Add("STATUS", "At least one order");
                        mv.Add("LASTORDER", DateTime.Now.ToShortDateString());
                        ...
                        EmailParameter results = mc.Subscribe(mailChimpListId, email, mv, "html", false, true, true, false);
                ...
                if (isGiftCard || paymentResponse.Status == "OK" || isPromoCode)
                {
                    var statusName = string.Empty;
                    statusName = (isGiftCard) ? OrderStatus.PaidWithGiftCard.ToString() : OrderStatus.PaymentSucess.ToString();
                    //Save/Subscribe new user on mailchimp list
                    var mailChimpAPIKey = GlobalConstants.MailChimpAPIKey;
                    var listId = GlobalConstants.MailChimpListId;
                    var mailChimpListId = string.IsNullOrEmpty(listId) ? mailChimpAPIKey : listId;
        ...
        private OrderVm GenerateOrderModel(RenderModel model)
        {
            ...
            {
                CustomerOrder = new Models.Entities.Order
                {
