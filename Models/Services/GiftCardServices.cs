        ...
        private GiftCardModel MakePayment(GiftCardModel model)
        {
            try
            {
                ...
                var checkOutRequest = new ScottybonsMVC.IcepayRestClient.Classes.Payment.CheckoutRequest
                {
                    ...
                    URLCompleted = "http://" + HttpContext.Current.Request.Url.Host + GlobalConstants.LanguageUrl + (currentLanguage.ToLower().Equals("nl")
                    ? GlobalConstants.IcePayGiftCardTranSuccessNl
                    : GlobalConstants.IcePayGiftCardTranSuccess),

                    //URLError = "http://61.12.8.212/ScottyBonsPhase2Test/en/giftcard",
                    URLError = "http://" + HttpContext.Current.Request.Url.Host + GlobalConstants.LanguageUrl + (currentLanguage.ToLower().Equals("nl")
                    ? GlobalConstants.IcePayGiftCardTranErrorNl
                    : GlobalConstants.IcePayGiftCardTranError),
